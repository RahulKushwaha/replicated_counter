...
anyNSuccessful(std::vector<folly::SemiFuture<T>> futures,
  ...
  std::shared_ptr<MultipleExceptions>
      multipleExceptions = std::make_shared<MultipleExceptions>();

  for (std::size_t index = 0; index < futures.size(); index++) {
    auto &future = futures[index];
    std::move(future)
        .via(&folly::InlineExecutor::instance())
        .then([successCounter, failureCounter, index,
                  n, promise,
          ...
          if (result.hasValue()) {
            ...
            multipleExceptions->
                add(index, result.exception().to_exception_ptr());
            (*failedValue)++;
          }
